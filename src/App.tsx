...
import { Info, X, Atom, Menu, MessageSquare } from 'lucide-react'; // Keep only used icons
import {
...
import Chatbot from './components/Chatbot'; // Import Chatbot component (adjust path if needed)
...
const RenderSimpleMarkdown: React.FC<{ text: string | string[]; baseTextColor?: string; className?: string }> = ({ text, baseTextColor = 'text-gray-700', className = '' }) => {
     const processText = (inputText: string): React.ReactNode[] => {
        return inputText.split('\n').map((paragraph, index) => {
            ...
            if (trimmed.startsWith('**')) {
                 ...
                 return <p key={index} className="font-semibold text-base md:text-lg mt-2 mb-0.5 text-gray-800">{content}</p>;
            }
             if (trimmed.startsWith('- ')) {
             ...
             if (trimmed.length > 0) {
                ...
                return (
                    <p key={index} className="my-1 text-sm md:text-base"> {/* Increased from text-xs/text-sm */}
                        {parts.map((part, i) =>
    ...
    const renderNodes = Array.isArray(text)
        ? text.map((item) => processText(item)).flat() // Removed unused idx
        : processText(text);
...
const VisionSection: React.FC<{ vision: VisionStatement }> = ({ vision }) => (
    <div className="bg-white rounded-xl border border-gray-200 shadow-xl p-5 md:p-8 h-full flex flex-col animate-fade-in">
        ...
        <div className="flex-grow overflow-y-auto pr-2 -mr-2 space-y-4 scrollbar-thin scrollbar-thumb-gray-300 hover:scrollbar-thumb-gray-400 scrollbar-track-transparent">
            ...
            <p className="text-xl md:text-2xl font-medium text-gray-800 mb-6 max-w-4xl mx-auto text-center">{vision.missionStatement}</p>
...
const TabButton: React.FC<{ isActive: boolean; onClick: () => void; children: React.ReactNode }> = ({ isActive, onClick, children }) => (
    <button
        onClick={onClick}
        className={`px-4 py-2 text-base md:text-lg font-medium rounded-md transition-colors duration-200 focus:outline-none focus-visible:ring-2 focus-visible:ring-indigo-500 focus-visible:ring-offset-1 ${
            isActive
    ...
    >
        <div>
            <div className="flex items-center mb-2">
                ...
                <h3 className={`text-lg md:text-xl font-bold ${pillar.textColor}`}>{pillar.title}</h3>
            </div>
            <p className={`text-sm md:text-base ${isSelected ? 'text-gray-700' : pillar.textColor} ${isSelected ? '' : 'opacity-80'}`}>{pillar.summary}</p> {/* Use summary */}
        </div>
         <span className={`mt-2 text-sm font-medium ${pillar.textColor} self-end ${isSelected ? 'opacity-100' : 'opacity-0'} group-hover:opacity-100 transition-opacity`}>
...
const PillarDetailView: React.FC<{ category: Category | null; onClose: () => void }> = ({ category: pillar, onClose }) => { // Use Category type, keep 'pillar' prop name for simplicity internally
    // const [showTechnical, setShowTechnical] = useState(false); // Removed unused state
    const detailContentRef = useRef<HTMLDivElement>(null);

    useEffect(() => {
        // Reset technical view when pillar changes
        // setShowTechnical(false); // Removed call to unused setter
        ...
        <div className="bg-white rounded-xl border border-gray-200 shadow-xl p-5 md:p-6 flex flex-col h-full relative animate-fade-in"> {/* Added animation */}
            ...
            <div className={`flex items-center mb-4 pb-3 border-b ${pillar.borderColor}`}>
                ...
                <div>
                    ...
                    <p className={`text-base md:text-lg ${pillar.textColor} opacity-80`}>{pillar.summary}</p> {/* Use summary */}
                </div>
            </div>
            ...
            <div ref={detailContentRef} className="flex-grow overflow-y-auto pr-2 -mr-2 space-y-4 scrollbar-thin scrollbar-thumb-gray-300 hover:scrollbar-thumb-gray-400 scrollbar-track-transparent"> {/* Custom scrollbar */}
                 {/* Enables Section */}
                 <div>
                    <h4 className={`text-base font-semibold mb-1 ${pillar.textColor}`}>Purpose:</h4> {/* Use purpose */}
                ...
                {/* Removed Technical Details Toggle Button */}
                 ...
                 <div className="space-y-1 animate-fade-in-fast">
                    <h4 className={`text-base font-semibold mb-2 ${pillar.textColor}`}>Sub-Components:</h4>
                         ...
                         {pillar.subComponents.map((sub: SubComponent) => (
                            <div key={sub.id} className="flex items-start space-x-2 p-1.5 rounded hover:bg-gray-50">
                                ...
                                <div className="pl-1"> {/* Add some indent */}
                                    <p className="font-medium text-sm text-gray-800 leading-tight">{sub.title}</p>
                                    <p className="text-sm text-gray-500 leading-tight">{sub.explanation}</p>
                                </div>
                            </div>
                         ))}
                     </div>
         ...
         <div className="flex-grow overflow-y-auto space-y-3 pr-2 -mr-2 scrollbar-thin scrollbar-thumb-gray-300 hover:scrollbar-thumb-gray-400 scrollbar-track-transparent">
             {concepts.map((concept) => (
                <div key={concept.id} className="flex items-center space-x-3 p-2.5 rounded-lg border border-gray-100 bg-gray-50 hover:bg-gray-100 transition-colors">
                     ...
                     <div>
                        <h4 className="font-semibold text-base text-gray-800 leading-tight">{concept.title}</h4>
                        <p className="text-sm text-gray-500 leading-tight">{concept.explanation}</p> {/* Use explanation */}
                     </div>
                </div>
             ))}
        </div>
                     ...
                     return (
                        <div key={agent.id} className={`p-3 rounded-lg border-l-4 ${categoryColorClass} bg-gray-50 border border-gray-200 hover:bg-gray-100 transition-colors space-y-1`}>
                             <div className="flex items-center space-x-2">
                                 ...
                                 <h4 className={`font-bold text-base ${agent.textColor}`}>{agent.name}</h4>
                             </div>
                             <p className="text-xs font-medium text-gray-500 uppercase tracking-wide">{agent.role}</p>
                             <p className="text-sm text-gray-700">{agent.capabilities}</p> {/* Use capabilities */}
                             {agent.scenario && (
                               <div className="mt-2 p-2 bg-white rounded border border-gray-200">
                                 <RenderSimpleMarkdown text={agent.scenario} baseTextColor="text-gray-600" className="text-sm"/>
                               </div>
                             )}
                         </div>
                     );
            ...
            <div className="bg-white rounded-xl shadow-2xl p-6 md:p-8 max-w-3xl w-full max-h-[80vh] overflow-y-auto relative scrollbar-thin scrollbar-thumb-gray-300 hover:scrollbar-thumb-gray-400 scrollbar-track-transparent">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-x-6 gap-y-4">
                    {Object.entries(glossary).sort(([termA], [termB]) => termA.localeCompare(termB)).map(([term, definition]) => (
                        <div key={term}>
                            <p className="font-semibold text-base text-indigo-700">{term}:</p>
        ...
        <div className="flex flex-col min-h-screen bg-gradient-to-br from-indigo-50 via-purple-50 to-pink-50 font-sans">
            ...
            <header className="bg-white/80 backdrop-blur-md shadow-sm sticky top-0 z-20 px-4 sm:px-6 lg:px-8 py-3 border-b border-gray-200">
                <div className="max-w-7xl mx-auto flex justify-between items-center">
                    <h1 className="text-xl md:text-2xl font-bold text-indigo-700 flex items-center">
            ...
            {!isChatOpen && (
                <button
                    ...
                    className="fixed bottom-5 right-5 z-30 p-3 bg-indigo-600 text-white rounded-full shadow-lg hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2 transition-transform hover:scale-110 animate-fade-in"
